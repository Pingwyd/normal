...
export async function GET() {
    ...
    return NextResponse.json(
      {
        error: {
          code: "UNAUTHORIZED",
          message: "Sign in to load account preferences.",
        },
      },
      { status: 401 },
    );
  ...
  const response = await proxyAccountApi("/v1/accounts/me", accessToken, {
    method: "GET",
  });
  ...
  if (!response.ok || body.error) {
    return NextResponse.json(
      { error: body.error?.message ?? "Could not load account preferences." },
    ...
    return NextResponse.json(
      {
        error: {
          code: "UNAUTHORIZED",
          message: "Sign in to save account preferences.",
        },
      },
      { status: 401 },
    );
